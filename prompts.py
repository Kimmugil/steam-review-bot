...
def build_prompt(game_name, store_stats, recent_label, top_langs_str, news_text, review_text, user_feedback=""):
    ...
    7. final_summary_all, final_summary_recent, country_analysis의 summary 항목 맨 앞에 반드시 '[긍정]' 또는 '[부정]' 머리말을 붙일 것. 긍정 항목을 배열 앞쪽에 먼저 나열할 것.
    ...
    13. 스팀 공식 평가의 리뷰 수가 0개로 표시되는 것은 기술적 수집 방식에 의한 것이므로, 이를 '데이터 오류'나 '표본 부족' 등으로 분석 내용에 절대 포함하지 말 것. 오직 평점의 상태(예: 복합적, 매우 긍정적 등)가 나타내는 유저 민심의 방향성에만 집중하여 서술할 것.
