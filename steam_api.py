...
def fetch_steam_reviews(app_id, recent_days_val, release_date):
    ...
    for lang in top_langs_keys:
        ...
        filtered_all[lang] = [f"[{'👍' if r['is_positive'] else '👎'} | ⏱️ {r['playtime']}h | ID: {r['steam_id']}] {r['review']}" for r in all_revs][:40]
        if recent_days_val:
            rec_revs = fetch_lang_reviews(app_id, lang, day_range=recent_days_val)
            # [v2.2.5] AI 분석 대표성 향상을 위해 언어별 전달 리뷰 수를 20개 → 40개로 상향
            filtered_recent[lang] = [f"[{'👍' if r['is_positive'] else '👎'} | ⏱️ {r['playtime']}h | ID: {r['steam_id']}] {r['review']}" for r in rec_revs][:40]
