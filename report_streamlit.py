...
def gray_box(title, body_text=None, items=None):
    html = '<div style="background:var(--color-background-secondary);border-radius:10px;padding:1.2rem 1.4rem;margin-bottom:1rem;">'
    if title:
        html += f'<div style="font-size:14px;font-weight:500;color:var(--color-text-primary);margin-bottom:9px;">{title}</div>'
    if body_text:
        html += f'<div style="font-size:16px;line-height:1.7;color:var(--color-text-secondary);">{body_text.replace(chr(10), "<br>")}</div>'
    if items:
        html += '<ul style="margin:0;padding-left:20px;">'
        for item in items:
            html += f'<li style="font-size:16px;line-height:1.65;margin-bottom:7px;color:var(--color-text-secondary);">{item}</li>'
        html += '</ul>'
    html += '</div>'
...
def render_report_tabs():
    ...
    with tab1:
        ...
        <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin-bottom:1.2rem;">
            <div style="background:var(--color-background-secondary);border-radius:10px;padding:1.1rem 1.2rem;">
                <div style="font-size:13px;color:var(--color-text-tertiary);margin-bottom:7px;">스팀 공식 평점</div>
                ...
                <div style="font-size:13px;color:var(--color-text-tertiary);margin-top:5px;">상점 직구매 유저 기준</div>
            </div>
            <div style="background:var(--color-background-secondary);border-radius:10px;padding:1.1rem 1.2rem;">
                <div style="font-size:13px;color:var(--color-text-tertiary);margin-bottom:7px;">전체 누적 평점</div>
                <div style="font-size:19px;font-weight:500;color:{eval_color(stats['all_desc'])};">{stats['all_desc']}</div>
                <div style="font-size:13px;color:var(--color-text-tertiary);margin-top:5px;">총 {stats['all_total']:,}개</div>
            </div>
            <div style="background:var(--color-background-secondary);border-radius:10px;padding:1.1rem 1.2rem;">
                <div style="font-size:13px;color:var(--color-text-tertiary);margin-bottom:7px;">{st.session_state.recent_label} 평점</div>
                <div style="font-size:19px;font-weight:500;color:{eval_color(stats['recent_desc'])};">{stats['recent_desc']}</div>
                <div style="font-size:13px;color:var(--color-text-tertiary);margin-top:5px;">표본 {stats['recent_total']:,}개</div>
            </div>
        </div>''', unsafe_allow_html=True)
        ...
        with c2:
            ...
            if period:
                st.markdown(f'<p style="font-size:13px;color:var(--color-text-tertiary);margin-bottom:8px;">📅 {period}</p>', unsafe_allow_html=True)
    ...
    with tab2:
        ...
        with c_news:
            ...
            if news and news[0]:
                ...
                st.markdown(f'''
                <div style="border:0.5px solid var(--color-border-tertiary);border-radius:10px;padding:1.1rem 1.2rem;margin-bottom:1rem;">
                    <div style="font-size:13px;color:var(--color-text-tertiary);margin-bottom:7px;">{news[3]}</div>
                    <a href="{news[2]}" target="_blank" style="font-size:16px;font-weight:500;color:var(--color-text-primary);text-decoration:none;line-height:1.5;">{news[0]}</a>
                </div>''', unsafe_allow_html=True)
                for line in ins.get('news_summary', []):
                    st.markdown(f'<div style="font-size:16px;line-height:1.65;margin-bottom:8px;color:var(--color-text-primary);">• {line}</div>', unsafe_allow_html=True)
            else:
                st.markdown(f'<p style="font-size:16px;color:var(--color-text-tertiary);">{ui.TEXTS["no_news"]}</p>', unsafe_allow_html=True)
    ...
    with tab3:
        sec("플레이타임별 민심 교차 분석")
        st.markdown(f'<p style="font-size:13px;color:var(--color-text-tertiary);margin-bottom:1rem;">{ui.TEXTS["tooltip_playtime"]}</p>', unsafe_allow_html=True)
        ...
        if pt:
            ...
            for col, tk, tot_k, avg_k, desc_k, sum_k, def_t in [
                ...
                with col:
                    st.markdown(f'''
                    <div style="background:var(--color-background-secondary);border-radius:10px;padding:1.1rem 1.2rem;margin-bottom:0.75rem;">
                        <div style="font-size:15px;font-weight:500;color:var(--color-text-primary);margin-bottom:5px;">{pt.get(tk, def_t)}</div>
                        <div style="font-size:13px;color:var(--color-text-tertiary);">표본 {stats.get(tot_k,0):,}개 · 평균 {stats.get(avg_k,0)}h · {stats.get(desc_k, ui.TEXTS["steam_eval_none"])}</div>
                    </div>''', unsafe_allow_html=True)
    ...
    with tab4:
        ...
        for reg in reg_data.get('regions', []):
            with st.expander(f"📍 {reg.get('region', '')}  —  {reg.get('trend', '')}"):
                ...
                if kws:
                    st.markdown(f'<p style="font-size:14px;color:var(--color-text-tertiary);margin-bottom:10px;">🔑 {", ".join(kws)}</p>', unsafe_allow_html=True)
        ...
        st.markdown(f'<p style="font-size:14px;color:var(--color-text-tertiary);margin-bottom:1rem;">{ui.TEXTS["country_analysis_desc"]}</p>', unsafe_allow_html=True)

        for country in ins.get('country_analysis', []):
            st.markdown(f'<p style="font-size:17px;font-weight:500;margin-top:1.5rem;margin-bottom:0.75rem;">🚩 {country.get("country", "")}</p>', unsafe_allow_html=True)
