...
def get_global_summary_block(ai_data, recent_label):
    ...
    for line in ai_data.get('final_summary_recent', []): 
        blocks.append({"object": "block", "type": "bulleted_list_item", "bulleted_list_item": {"rich_text": format_sentiment_line(line)}})
...
def get_playtime_analysis_block(ai_data):
    ...
    if comparison_insights and any(isinstance(line, str) and line.strip() for line in comparison_insights):
        # 💡 콜아웃 내부에 정식 리스트(목록) 블록으로 삽입! 워딩 '체크포인트'로 변경.
    ...
    for line in playtime_data.get('newbie_summary', ["데이터 부족"]): 
    ...
    for line in playtime_data.get('core_summary', ["데이터 부족"]): 
        blocks.append({"object": "block", "type": "bulleted_list_item", "bulleted_list_item": {"rich_text": [{"text": {"content": line}}]}})
