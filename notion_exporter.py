...
def get_global_summary_block(ai_data, recent_label):
    ...
    for line in ai_data.get('final_summary_all', []): blocks.append({"object": "block", "type": "bulleted_list_item", "bulleted_list_item": {"rich_text": [{"text": {"content": line}}]}})
    ...
    for line in ai_data.get('final_summary_recent', []): blocks.append({"object": "block", "type": "bulleted_list_item", "bulleted_list_item": {"rich_text": [{"text": {"content": line}}]}})
...
def get_playtime_analysis_block(ai_data):
    ...
    if comparison_insights and any(isinstance(line, str) and line.strip() for line in comparison_insights):
        blocks.append({"object": "block", "type": "callout", "callout": {"icon": {"emoji": "⚖️"}, "color": "gray_background", "rich_text": [{"text": {"content": "📊 핵심 교차 인사이트\n"}, "annotations": {"bold": True, "color": "gray"}}, {"text": {"content": "\n".join([line for line in comparison_insights if isinstance(line, str) and line.strip()])}}]}})
    ...
    for line in playtime_data.get('newbie_summary', ["데이터 부족"]): blocks.append({"object": "block", "type": "bulleted_list_item", "bulleted_list_item": {"rich_text": [{"text": {"content": line}}]}})
    blocks.append({"object": "block", "type": "heading_3", "heading_3": {"rich_text": [{"text": {"content": playtime_data.get('core_title', '💀 코어 유저 여론')}, "annotations": {"color": "purple", "bold": True}}]}})
    for line in playtime_data.get('core_summary', ["데이터 부족"]): blocks.append({"object": "block", "type": "bulleted_list_item", "bulleted_list_item": {"rich_text": [{"text": {"content": line}}]}})
    blocks.append({"object": "block", "type": "divider", "divider": {}})
    return blocks
...
def get_news_summary_block(news_data, ai_data):
    ...
    if isinstance(news_summary_data, str): news_summary_data = [news_summary_data]
    if news_summary_data and any(line.strip() for line in news_summary_data):
        for line in news_summary_data:
            if line.strip(): blocks.append({"object": "block", "type": "bulleted_list_item", "bulleted_list_item": {"rich_text": [{"text": {"content": line}}]}})
    else:
        blocks.append({"object": "block", "type": "bulleted_list_item", "bulleted_list_item": {"rich_text": [{"text": {"content": "상세 내용은 위 링크를 확인해주세요.", "annotations": {"color": "gray"}}}]}})
    blocks.append({"object": "block", "type": "divider", "divider": {}})
    return blocks
...
def get_language_ratio_block(store_stats):
    blocks = [{"object": "block", "type": "heading_2", "heading_2": {"rich_text": [{"text": {"content": "🌐 전 세계 누적 리뷰 작성 언어 비중"}}]}}, {"object": "block", "type": "paragraph", "paragraph": {"rich_text": [{"text": {"content": f"총 누적 리뷰 수: {store_stats['all_total']:,}개 (전체 언어 취합 기준)"}, "annotations": {"bold": True, "color": "gray"}}]}}]
    table_rows = [{"type": "table_row", "table_row": {"cells": [[{"text": {"content": "순위"}, "annotations": {"bold": True, "color": "gray"}}], [{"text": {"content": "언어"}, "annotations": {"bold": True, "color": "gray"}}], [{"text": {"content": "누적 리뷰 수"}, "annotations": {"bold": True, "color": "gray"}}], [{"text": {"content": "비중"}, "annotations": {"bold": True, "color": "gray"}}]]}}]
    ...
    for idx, (lang_code, count) in enumerate(sorted_langs):
        ...
        table_rows.append({"type": "table_row", "table_row": {"cells": [[{"text": {"content": f"{idx+1}위"}}], [{"text": {"content": get_lang_name(lang_code)}}], [{"text": {"content": f"{count:,}개"}}], [{"text": {"content": f"{ratio:.1f}%"}}]]}})
    ...
    blocks.append({"object": "block", "type": "paragraph", "paragraph": {"rich_text": [{"text": {"content": "언어 비중 표는 실제 플레이 유저 비례보다 영어 리뷰 비중이 높게 나타날 수 있습니다."}, "annotations": {"italic": True, "color": "gray"}}]}})
    blocks.append({"object": "block", "type": "divider", "divider": {}})
    return blocks

def get_country_analysis_block(ai_data):
    blocks = [{"object": "block", "type": "heading_2", "heading_2": {"rich_text": [{"text": {"content": "🌍 국가별 세부 평가 분석 (TOP 3 + 한국)"}}]}}]
...
def upload_to_notion(app_id, game_name, store_stats, ai_data, recent_label, smart_reason, news_data):
    ...
    # 2. 블록 조립 (Config에 정의된 순서대로!)
    children_blocks = []
    
    # [새로운 기능] 상단에 자동 목차(Table of Contents) 추가!
    children_blocks.append({"object": "block", "type": "table_of_contents", "table_of_contents": {"color": "default"}})
    ...
    for i in range(0, len(children_blocks), 100):
        res = requests.patch(append_url, headers=headers, data=json.dumps({"children": children_blocks[i:i+100]}))
        res.raise_for_status()
        time.sleep(0.5) 
