...
def main():
    ...
    with st.sidebar:
        ...
        if not st.session_state.history:
            st.caption("아직 분석한 게임이 없습니다.")
        else:
            for h in reversed(st.session_state.history[-5:]): # 최근 5개만 역순으로 노출
                st.markdown(f"- **{h['name']}**")
        st.divider()
        st.caption(f"Version: {APP_VERSION}")
        with st.expander("🛠️ 업데이트 이력"): st.markdown(UPDATE_HISTORY)

    # 헤더 영역 (타이틀과 노션 링크를 나란히 배치)
    ...
    if st.session_state.step == 0:
                ...
                with st.status("데이터를 수집하고 분석하는 중... 🌾", expanded=True) as status:
                    try:
                        ...
                        if not rid:
                            ...
                            st.error("게임 정보를 찾을 수 없습니다."); return
                        p.progress(20)
                        ...
                        # 성공 시 세션 데이터 업데이트 및 히스토리에 게임 추가
        ...
        st.subheader(f"[{st.session_state.game_name}] 분석 결과 미리보기")
