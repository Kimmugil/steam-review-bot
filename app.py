...
import time
import threading
from config import APP_VERSION, ENV_NAME, NOTION_PUBLIC_URL  # 💡 변수명 통일 완료!
...
def main():
    ...
    with col_link:
        # 💡 [통일] config에서 가져온 NOTION_PUBLIC_URL을 사용
    ...
    # --- Step 2: 결과 검수 ---
            ...
            for country in ai_data.get('country_analysis', []):
                with st.expander(f"🚩 {country['language']}"):
                    for cat in country['categories']:
                        st.write(f"**{cat['name']}**")
                        for line in cat['summary']: st.write(f"- {line}")
        ...
        with col1:
            if st.button("🔄 다시 분석하기"):
                st.session_state.step = 1 # 단순화를 위해 1단계로 회귀
                st.rerun()
        with col2:
